...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  MeasureGroup meas = lidar_meas.measures.back();// get the last measure
  ...
  if (imu_need_init)
  {
    ...
    last_imu = meas.imu.back();//记录了当前测量组中的最后一帧 IMU 数据
  ...
  UndistortPcl(lidar_meas, stat, *cur_pcl_un_);//对点云进行去畸变处理
