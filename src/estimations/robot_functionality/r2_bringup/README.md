...
ros2 launch r2_bringup simulation_bringup.launch.py start_sim:=true mode:=nav
```
...
ros2 launch r2_bringup dynamic_waypoint_mission.launch.py start_sim:=false
...
ros2 launch r2_bringup simulation_with_yolo_bringup.launch.py start_sim:=true
...
```bash
# 1. 启动仿真环境和导航
ros2 launch r2_bringup simulation_bringup.launch.py start_sim:=true
...
2. 确认目标坐标系和TF
  ...
  - map→odom 和 odom→base_link 在启动后均稳定可用
3. 话题检查
