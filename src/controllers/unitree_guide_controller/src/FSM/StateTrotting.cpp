...
void StateTrotting::calcGain() const {
    for (int i(0); i < 4; ++i) {
        if (wave_generator_->contact_(i) == 0) {
            // swing gain
            for (int j = 0; j < 3; j++) {
                const double kp_val = ctrl_interfaces_.use_sim_kp_kd_ ? 3.0 : (3.0 / 40.0);
                const double kd_val = ctrl_interfaces_.use_sim_kp_kd_ ? 2.0 : (2.0 / 40.0);
                ctrl_interfaces_.joint_kp_command_interface_[i * 3 + j].get().set_value(kp_val);
                ctrl_interfaces_.joint_kd_command_interface_[i * 3 + j].get().set_value(kd_val);
            }
        } else {
            // stable gain
            for (int j = 0; j < 3; j++) {
        ...
        //     // swing gain
        //     for (int j = 0; j < 3; j++) {
        //         ctrl_interfaces_.joint_kp_command_interface_[i * 3 + j].get().set_value(3.0);
